...
For more details and the beginnings of an empirical investigation using this framework, see the working paper in `projects/paper/`.
...
projects/              subprojects (each is its own git repo, gitignored here)
  paper/               working paper, notebooks, training configs, and data
```
...
1.  **LLM Training**: `projects/paper/data/<dataset>/` → `train.py` → `out-\<dataset\>/ckpt.pt`
2.  **Corpus Run**: `shape.extract.extract_features` → `features/<dataset>/` (per-position $h$ or $h_⊥$)
...
5.  **Global Density Estimation**: `shape.density.fit_flow` → `models/<dataset>/` (Neural Spline Flow fit on $h$ or $h_⊥$)
...
python scripts/extract_features.py  --ckpt out-coca/ckpt.pt \
                                    --data projects/paper/data/coca/test.bin \
...
`shape.extract.valid_positions(meta)` maps memmap rows to corpus indices. `shape.viz` builds a working subsample with cached `log_Z_of_h` so that $p_w(h_i)$ is an O(d) query per token, which is what makes interactive polysemy plotting tractable.

## Quickstart on the Toy Corpus

``` bash
python projects/paper/data/shakespeare_char/prepare.py
python train.py projects/paper/config/train_shakespeare_char.py
python scripts/extract_features.py  --ckpt out-shakespeare-char/ckpt.pt \
       --data projects/paper/data/shakespeare_char/val.bin \
       --out-dir features/shakespeare_char --dataset shakespeare_char
...
Then open `projects/paper/notebooks/shakespeare_char_polysemy.ipynb` for a walk-through of the density and visualization layers on the same features. The COCA-scale counterpart is `projects/paper/notebooks/coca_polysemy.ipynb`.
